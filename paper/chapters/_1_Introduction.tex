...
Collision detection is used in applications such as computer graphics, physics simulations and video games.
...
If two bounding boxes do not overlap, neither will the primitive objects enclosed in them, allowing to skip the otherwise needed exact collision checks. \cite[pp.~75--81]{ericson2004real}
...
This work investigates the influence of this behavior on traversal cost by comparing one representative of each of three general construction mechanisms: top-down, bottom-up, and incremental construction.
The following research question will be addressed by this WAB:

\begin{quote}
\textit{How do the evaluated top-down, bottom-up and incremental BVH construction implementations influence the number of bounding-volume checks required for all-pairs collision detection in static two-dimensional particle distributions?}
\end{quote}
